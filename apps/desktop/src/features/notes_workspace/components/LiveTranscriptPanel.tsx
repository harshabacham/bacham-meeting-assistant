        ...
        return (
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-50">
                ...
                >
                    ...
                    <div className="flex items-center bg-[var(--surface)] dark:bg-[#1c1c1a] border border-[var(--border)] rounded-full shadow-md pl-4 pr-1.5 py-1 min-w-[380px]">
                        <input
                            type="text"
    ...
    return (
        <div className="fixed inset-x-0 bottom-6 flex flex-col items-center justify-center z-50 pointer-events-none px-4 gap-2">
            ...
            <motion.div 
                ...
                animate={{ opacity: 1, y: 0, scale: 1 }}
                exit={{ opacity: 0, y: 20 }}
                className="w-full max-w-xl bg-[var(--surface)] dark:bg-[#181816] border border-[var(--border)] rounded-[1.75rem] shadow-2xl overflow-hidden flex flex-col pointer-events-auto"
            >
                {/* Header (Image 1) */}
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--border)] bg-[var(--surface)] dark:bg-[#181816]">
                    <button className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors p-1">
                        <Search size={14} />
                    </button>
                    
                    <span className="text-[11px] text-[var(--text-muted)] font-sans cursor-pointer hover:underline">
                        Always get consent when transcribing others. Learn more &gt;
                    </span>

                    <div className="flex items-center gap-1 text-[var(--text-muted)]">
                        <button className="hover:text-[var(--text-primary)] p-1 transition-colors">
                            <ThumbsDown size={13} />
                        </button>
                        <button onClick={handleCopyAll} className="hover:text-[var(--text-primary)] p-1 transition-colors" title="Copy transcript">
                            {copied ? <Check size={13} className="text-emerald-500" /> : <Copy size={13} />}
                        </button>
                        <button onClick={() => setIsMinimized(true)} className="hover:text-[var(--text-primary)] p-1 transition-colors" title="Minimize">
                            <Minus size={14} />
                        </button>
                    </div>
                </div>
                ...
                <div 
                    ref={scrollRef}
                    className="p-4 max-h-[38vh] min-h-[160px] overflow-y-auto space-y-2.5 bg-[var(--surface)] dark:bg-[#181816] font-sans"
                >
                    ...
                    <AnimatePresence>
                        {chunks.map((chunk, idx) => (
                            ...
                            >
                                {idx === 2 && (
                                    <div className="text-[10px] text-[var(--text-muted)] text-center my-1.5 font-mono">
                                        {formatTime(recordingTime)}
                                    </div>
                                )}
                                
                                <div className="bg-[#f4f4ef] dark:bg-[#252523] border border-transparent dark:border-[var(--border)] rounded-xl px-3.5 py-2.5 text-[13px] leading-relaxed text-[#2c2c2c] dark:text-[#eaeae6] max-w-[92%]">
                                    {chunk.text}
                                </div>
                            </motion.div>
                        ))}
                    ...
                    {interimText && (
                        <motion.div 
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="bg-[#f4f4ef] dark:bg-[#252523] border border-transparent dark:border-[var(--border)] rounded-xl px-3.5 py-2.5 text-[13px] text-[var(--text-muted)] italic max-w-[92%]"
                        >
                            {interimText} <span className="animate-pulse text-[var(--text-muted)]">...</span>
                        </motion.div>
                    )}
                </div>

                {/* Bottom Control Bar (Image 1) */}
                <div className="flex items-center justify-between px-4 py-2.5 border-t border-[var(--border)] bg-[var(--surface)] dark:bg-[#181816]">
                    {/* Left: Waveform & Stop */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-medium px-2 py-1 rounded hover:bg-[var(--surface-hover)] transition-colors"
                            >
                                <Languages size={13} className="text-[var(--text-muted)]" />
                                <span>
                            ...
                            {isLangMenuOpen && (
                                <div className="absolute bottom-full right-0 mb-1.5 w-44 bg-[var(--surface)] dark:bg-[#1c1c1a] border border-[var(--border)] rounded-xl shadow-xl z-50 py-1 max-h-56 overflow-y-auto">
                                    {LANGUAGES.map(lang => (
                                        <button
                                            key={lang.code}
                                            onClick={() => handleLanguageChange(lang.code)}
                                            className={`w-full text-left px-3 py-1.5 text-xs flex items-center justify-between hover:bg-[var(--surface-hover)] transition-colors ${selectedLanguage === lang.code ? 'text-[#3d5a22] dark:text-[#95d648] font-bold bg-[#f4f7ee] dark:bg-[#283818]' : 'text-[var(--text-primary)]'}`}
                                        >
                                            <span>{lang.label}</span>
                                            {selectedLanguage === lang.code && <span className="w-1.5 h-1.5 rounded-full bg-[#3d5a22] dark:bg-[#95d648]" />}
                                        </button>
                                    ))}
                                </div>
                            )}
