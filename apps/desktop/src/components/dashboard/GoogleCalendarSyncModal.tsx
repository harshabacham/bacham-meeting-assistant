...
import { X, Calendar, RefreshCw, Link as LinkIcon, Mail, ShieldCheck, LogOut, Plus } from 'lucide-react';
...
export function GoogleCalendarSyncModal() {
  ...
  const handleConnect = async (e: React.FormEvent) => {
    ...
    if (emailInput.trim()) {
      try {
        await connectGoogleCalendar(emailInput.trim(), iCalInput.trim() || undefined);
      } catch (err: any) {
        setLocalError(err.message || "Failed to connect. Please verify your Google Calendar iCal feed URL.");
      }
    }
  };
              ...
              <div className="space-y-5">
                ...
                {!isConnected ? (
                  <div className="space-y-4 pt-1">
                    {/* Primary Option: Google Calendar OAuth Sign-In */}
                    <button
                      type="button"
                      onClick={async () => {
                        setLocalError(null);
                      ...
                      disabled={isSyncing}
                      className="w-full py-3 bg-primary text-primary-foreground rounded-xl text-xs font-bold hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-md active:scale-[0.99] disabled:opacity-50"
                    >
                      {isSyncing ? (
                    ...
                    <div className="relative flex items-center justify-center py-2">
                      <div className="border-t border-border/40 w-full absolute" />
                      <span className="bg-surface px-3 text-[10px] text-muted-foreground uppercase font-bold tracking-wider relative z-10">
                        Advanced: iCal Address
                      </span>
                    </div>

                    {/* Secondary Option: Manual iCal URL Input Form */}
                    <form onSubmit={handleConnect} className="space-y-4">
                      <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                          <Mail size={12} /> Google Account Email
                        </label>
                        <input
                          type="email"
                          value={emailInput}
                          ...
                          className="w-full px-3.5 py-2.5 text-xs bg-[#1A1C20] border border-border/80 rounded-xl text-white placeholder:text-muted-foreground/60 outline-none focus:border-primary transition-all font-mono"
                        />
                      </div>

                      <div className="space-y-1.5">
                        <label className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
                          <LinkIcon size={12} /> Secret iCal Address URL
                        </label>
                        <input
                          type="url"
                          value={iCalInput}
                          ...
                          className="w-full px-3.5 py-2.5 text-xs bg-[#1A1C20] border border-border/80 rounded-xl text-white placeholder:text-muted-foreground/60 outline-none focus:border-primary transition-all font-mono"
                        />
                        <div className="p-3 bg-surface-raised/40 border border-border/30 rounded-xl text-[11px] text-muted-foreground leading-relaxed space-y-1.5 mt-1">
                          <p className="font-semibold text-foreground">How to get your Secret iCal URL (100% Free):</p>
                          <ol className="list-decimal list-inside space-y-1 pl-1">
                            <li>Open <a href="#" onClick={(e) => { e.preventDefault(); openUrl("https://calendar.google.com").catch(console.error); }} className="text-primary hover:underline font-medium">Google Calendar</a> in your web browser.</li>
                            <li>On the left sidebar, hover over your main calendar, click the <strong>three dots (⋮)</strong>, and select <strong>Settings and sharing</strong>.</li>
                            <li>Scroll all the way down to the <strong>Integrate calendar</strong> section.</li>
                            <li>Copy the <strong>Secret address in iCal format</strong> URL (make sure it says <em>Secret</em>, not Public!).</li>
                          </ol>
                        </div>
                      </div>
                      ...
                      <button
                        type="submit"
                        disabled={isSyncing}
                        className="w-full py-2.5 bg-surface border border-border hover:bg-surface-hover text-foreground rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm active:scale-[0.99] disabled:opacity-50"
                      >
                        <ShieldCheck size={14} /> Connect via iCal
                      </button>
                    </form>
                  </div>
                ) : (
