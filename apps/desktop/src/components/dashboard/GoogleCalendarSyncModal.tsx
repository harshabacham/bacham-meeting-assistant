              ...
              <div className="space-y-5">
                ...
                {!isConnected ? (
                  <div className="space-y-4 pt-1">
                    {deviceFlowData ? (
                      <div className="flex flex-col items-center justify-center p-4 bg-surface-raised border border-border/50 rounded-xl space-y-3">
                        <div className="text-center space-y-1">
                          <p className="text-[11px] text-muted-foreground">Enter this code on your device</p>
                          <p className="text-3xl font-mono font-bold tracking-widest text-primary selection:bg-primary/20">{deviceFlowData.userCode}</p>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            openUrl(deviceFlowData.verificationUrl).catch(console.error);
                          }}
                          className="w-full py-2.5 bg-primary text-primary-foreground rounded-lg font-bold hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-sm"
                        >
                          Open Google Login
                        </button>
                        <div className="w-full flex items-center justify-between text-[10px] text-muted-foreground pt-2 border-t border-border/40">
                          <span className="flex items-center gap-1.5"><RefreshCw size={10} className="animate-spin" /> Waiting for approval...</span>
                          <button onClick={cancelDeviceFlow} className="hover:text-destructive underline decoration-dotted underline-offset-2">Cancel</button>
                        </div>
                      </div>
                    ) : (
                      <button
