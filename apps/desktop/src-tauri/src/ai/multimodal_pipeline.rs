...
Return ONLY valid JSON.
{
  ...
  "key_concepts": ["string"],
  "crm_metadata": null // OPTIONAL: Only include { bant: {}, action_items: [{task, owner, due_date, timestamp, status, reasoning}], key_decisions: [] } if action items are discussed OR written on slides/screens. Always extract visually written deadlines into action_items even if not spoken aloud. Otherwise null.
}
